...
local function factory(args)
	...
	function cal.build(month, year)
		...
		for x = 1, mth_days do
			local strx = x ~= today and x
				or markup.bold(markup.color(cal.notification_preset.bg, cal.notification_preset.fg, x) .. " ")
	...
	function cal.show(timeout, month, year, scr)
		...
		if cal.three then
			...
			cal.notification_preset.text = string.format(
				"%s\n\n%s\n\n%s",
				tconcat(cal.build(prev_month, prev_year)),
				cal.notification_preset.text,
