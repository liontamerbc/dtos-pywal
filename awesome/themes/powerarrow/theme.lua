...
-- Scroll to change volume (mouse wheel over volume widget)
local function dtos_change_volume(step)
    local channel = (theme.volume and theme.volume.channel) or "Master"
    awful.spawn(string.format("amixer -q set %s %s", channel, step), false)
...
volicon:buttons(my_table.join(
    awful.button({}, 4, function() dtos_change_volume("2%+") end),
    awful.button({}, 5, function() dtos_change_volume("2%-") end)
))

if theme.volume and theme.volume.widget and theme.volume.widget.buttons then
