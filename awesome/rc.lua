...
-- Override wallpaper from Awesome cache if available
do
    local home = os.getenv("HOME") or ""
    local cache_path = home .. "/.cache/wall_awesome"
    local f = io.open(cache_path, "r")
    if f ~= nil then
        local line = f:read("*l")
        f:close()
        if line ~= nil and line ~= "" then
            beautiful.wallpaper = line
        end
    end
end
...
-- Original DTOS wallpaper autostart disabled (used ~/.cache/wall)
-- awful.spawn.with_shell("xargs xwallpaper --stretch < ~/.cache/wall")
awful.spawn.with_shell("if [ -f $HOME/.cache/wall_awesome ]; then xwallpaper --stretch \"$(cat $HOME/.cache/wall_awesome)\"; fi")
